...
**Per-session approved.json files:**
...
approved variants. Use these to inform new generation — if the profile says "user prefers
Geist font" (from aggregated history), the approved.json files add the specific recent
approval context.
...
**Launch N subagents in a single message** (parallel execution). Each agent is independent
...
Open the comparison board in the user's browser:

```bash
$D compare --images "$_IMAGES" --output "$_DESIGN_DIR/comparison.html"
open "$_DESIGN_DIR/comparison.html"
```

Then enter the feedback loop:

1. Ask the user which variant they prefer and why
2. If they want refinements, generate new variants incorporating feedback
3. Repeat until they approve a direction
4. Save the approved variant info to `approved.json`
...
```json
{
  "approved_variant": "variant-{letter}.png",
  "ratings": { "A": 4, "B": 3, "C": 2 },
...
1. **Never save to `.context/`, `docs/designs/`, or `/tmp/`.** All design artifacts go
   to `~/.gstack/projects/$SLUG/designs/`.
